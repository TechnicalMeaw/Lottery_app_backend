...
def get_lottery_time_left_in_millis():
    now = datetime.now()
    nine_pm = datetime(now.year, now.month, now.day+1, 0, 5, 0)  # Set the time to 9 PM
...
def is_lottery_active(timeZoneOffset):
    ...
    now = datetime.now()
    nine_pm = datetime(now.year, now.month, now.day, 4, 0, 0)  # Set the time to 9 PM
...
def delete_prev_lottery_data(db: Session, timeZoneOffset):
    ...
    db.query(models.Lottery).filter(models.Lottery.created_at < datetime(now.year, now.month, now.day, 4, 0, 0)).delete(synchronize_session=False)
